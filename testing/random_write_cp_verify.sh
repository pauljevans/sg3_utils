...
if ! INQ=$(sg_inq --maxlen=36 "${1}" 2>/dev/null) ; then
...
if ! INQ=$(sg_inq --maxlen=36 "${2}" 2>/dev/null) ; then
...
if ! ${dd_clone} iflag=random of="${1}" "${SDP_OPTS[@]}" ; then
	exit
fi

# Copy $1 to $2
echo ${dd_clone} if="${1}" of="${2}" "${SDP_OPTS[@]}"
if ! ${dd_clone} if="${1}" of="${2}" "${SDP_OPTS[@]}" ; then
	exit
fi
...
if ! ${dd_clone} --verify if="${1}" of="${2}" "${SDP_OPTS[@]}" ; then
	exit
fi
